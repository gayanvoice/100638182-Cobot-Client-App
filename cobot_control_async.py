...
async def main(set_position_array):
    host = "localhost"
    port = 30004
    ...
    set_position = rtde_connection.send_input_setup(setp_names, setp_types)
    ...
    control_task = asyncio.create_task(control_cobot(rtde_connection, watchdog, set_position_array, set_position))
...
async def control_cobot(rtde_connection, watchdog, set_position_array, set_position):
    ...
    while keep_running:
        ...
        if move_completed and state.output_int_register_0 == 1:
            ...
            rtde_connection.send(set_position)
