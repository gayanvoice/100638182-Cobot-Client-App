...
class Wrist2(object):
    ...
    def stdin_listener():
        while True:
            selection = input("Press C to quit Wrist2\n")
            if selection == "C" or selection == "c":
                print("Quitting Wrist2...")
                break

    def iot_task_callback(self, values):
        if self.__iot_lock:
            logging.info("wrist2.iot_task_callback:__iot_lock={iot_lock}"
            ...
            logging.error("wrist2.iot_task_callback:__iot_lock={iot_lock}"
                          .format(iot_lock=self.__iot_lock))

    async def start_iot_command_handler(self, values):
        if values:
            logging.info("wrist2.start_iot_command_handler:values={values} type={type}"
    ...
    async def stop_iot_command_handler(self, values):
        if values:
            self.__iot_lock = True
            logging.info("wrist2.stop_iot_command_handler:values={values} type={type}"
                         .format(values=values, type=str(type(values))))
