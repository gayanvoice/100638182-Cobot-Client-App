...
class Payload(object):
    ...
    def iot_task_callback(self, values):
        if self.__iot_lock:
            logging.info("payload.iot_task_callback:__iot_lock={iot_lock}"
            ...
            logging.error("payload.iot_task_callback:__iot_lock={iot_lock}"
                          .format(iot_lock=self.__iot_lock))

    async def start_iot_command_handler(self, values):
        if values:
            logging.info("payload.start_iot_command_handler:values={values} type={type}"
    ...
    async def stop_iot_command_handler(self, values):
        if values:
            self.__iot_lock = True
            logging.info("payload.stop_iot_command_handler:values={values} type={type}"
                         .format(values=values, type=str(type(values))))
