...
from cloud.iot_task.tool_iot_task import ToolIotTask
...
class Tool(object):
    ...
    def stdin_listener(self):
        while True:
            ...
            process_continue = tool_configuration.find('status').text
            if process_continue == "False":
                logging.info("tool.stdin_listener:break process_continue={process_continue}"
    ...
    def iot_task_callback(self, values):
        if self.__iot_lock:
            logging.info("tool.iot_task_callback:__iot_lock={iot_lock}"
                         .format(iot_lock=self.__iot_lock))
            self.__iot_lock = False
            self.__iot_task = ToolIotTask(self.__device)
            ...
            logging.error("tool.iot_task_callback:__iot_lock={iot_lock}"
                          .format(iot_lock=self.__iot_lock))

    async def start_iot_command_handler(self, values):
        if values:
            logging.info("tool.start_iot_command_handler:values={values} type={type}"
    ...
    async def stop_iot_command_handler(self, values):
        if values:
            self.__iot_lock = True
            logging.info("tool.stop_iot_command_handler:values={values} type={type}"
                         .format(values=values, type=str(type(values))))
    ...
    async def connect_azure_iot(self, queue):
        ...
        if not command_listeners.done():
            command_listeners.set_result(["Tool done"])

        command_listeners.cancel()

        await self.__device.iot_hub_device_client.shutdown()
        logging.info("tool.connect_azure_iot:queue.put")
